            ...
            if (item.name == 'APP_LAYOUT_WEBVIEW_ENABLED') {
                input.setValidator(value => {
                    if (!config.app_layout_webview && value) {
                        throw new Error('DISEÑO DE WEBVIEW No puede estar vacío.')
                    }
                });
            }
...
const main = async () => {
    ...
    appConfigList.observe(async (event, config) => {
        if (event == 'update') {
            ...
            if (response.status === 200) {
                showToastSuccess('¡La configuración se guardó correctamente!');
                return;
            }
    ...
    appConfigList.observe(async (event, config) => {
        if (event == 'delete') {
            ...
            if (response.status === 204) {
                showToastSuccess('¡Configuración eliminada exitosamente!');
                main();
                return;
            }
    ...
    appConfigList.observe(async (event, data) => {
        if (event == 'create') {
            ...
            if (response.status == 201) {
                showToastSuccess('Layout creado exitosamente!');
                main();
                return;
            }
            ...
            showToastError('Error al crear el layout!');
        }
    });
    ...
    appConfigList.observe(async (event, id) => {
        if (event == 'toggle') {
            ...
            if (response.status === 200) {
                showToastSuccess('La configuración cambió exitosamente!');
                main();
                return;
            }
    ...
    appConfigList.observe(async (event, config) => {
        if (event == 'import') {
            ...
            if (response.status == 201) {
                showToastSuccess('Configuración importada exitosamente!');
                return;
            }
    ...
    appConfigList.observe(async (event, config) => {
        if (event === 'sync') {
            ...
            if (result.data && result.status != 201) {
                showToastError('¡Error al sincronizar la lista de configuración!');
                return;
            }

            showToastSuccess('Felicitaciones, la configuración se sincronizó correctamente!');
            showLoading();
            getConfigApp();
        }
    });
